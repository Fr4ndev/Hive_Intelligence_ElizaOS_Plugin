...
import TelegramClient from "@ai16z/client-telegram";
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (plugin && plugin.name && plugin.description) {
          // Initialize Hive plugin if it\'s the hive plugin
          if (plugin.name === 'hive' && plugin.initialize) {
            // We\'ll initialize it later when we have the character config
          }
...
async function main() {
  ...
  // validateCharacterConfig(character); // Comentado para permitir la ejecución sin modelProvider
  const localPlugins = await loadLocalPlugins();
  console.log(
  ...
  for (const character of characters) {
    ...
    const runtime = new AgentRuntime({
      character,
      plugins: combinedPlugins.map(p => {
        if (p.name === 'hive' && (p as any).initialize) {
          (p as any).initialize(character);
        }
        return p;
      }),
      token: "dummy-token",
    ...
    if (character.clients.includes(Clients.TELEGRAM)) {
      const telegramBotToken = character.settings?.secrets?.TELEGRAM_BOT_TOKEN;
      if (!telegramBotToken) {
        elizaLogger.error(`TELEGRAM_BOT_TOKEN not found for character ${character.name}.`);
        continue;
      }
      const telegramClient = TelegramClient;
      telegramClient.start(runtime);
      elizaLogger.success(`Telegram client started for agent "${character.name}"!`);
    }
  }
